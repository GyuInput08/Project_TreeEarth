		...
		$('.qa_li .question').click(
				function() {
					...
					if (q != qnaNum) { // Q눌렀을 때 문의 글 상세내용 보이도록 스르륵 동작
						...
						$('.qa_li .answer').eq(qnaNum).stop(true, true)
								.slideDown(400);
						TweenMax.to($('.qa_li .question').eq(qnaNum).find(
								'.iconDiv'), 0.4, {
							rotation : 0
						...
						TweenMax.to($('.qa_li').eq(qnaNum).find('.question p'),
								0.4, {
									rotation : 0
								});
						qnaNum = -1;
...
<body>
	...
	<div>
		<ul class="listWrap">
			<c:choose>
				<c:when
					test="${not empty storeQnaList and pageInfo.itemListCount gt 0}">
					<c:forEach var="store_qna" items="${storeQnaList }">
						<li class="qa_li">
							<div class="question">
								<p class="tit">${store_qna.mem_id }님의문의입니다.</p>
								<p>&nbsp;&nbsp;&nbsp;&nbsp;${store_qna.sto_qna_date }</p>
								<p class="iconDiv">
									<img src="https://happyjung.diskn.com/data/lecture/icon_jquery_faq2_icon_arrow.png">
								</p>
							</div>
							<div class="answer">
